 ...
 */
public class PreliminaryCountValidator implements CountValidator<PreliminaryCount> {
	...
	@Override
	public void applyValidationRules(PreliminaryCount count, CountContext context, MvArea countingArea, CountingMode countingMode,
		...
		if (reportingUnitTypeId == ReportingUnitTypeId.VALGSTYRET && !validCountingModesForValgstyret.contains(countingMode)) {
			throw new ValidateException(format("illegal count mode for preliminary count: <%s> when reporting unit is <%s>", countingMode.getDescription(),
					reportingUnitTypeId.name()));
		}

		if (reportingUnitTypeId == ReportingUnitTypeId.STEMMESTYRET && countingMode != CountingMode.BY_POLLING_DISTRICT) {
			throw new ValidateException(format("illegal count mode for preliminary count: <%s> when reporting unit is <%s>", countingMode.getDescription(),
					reportingUnitTypeId.name()));
		}
